...
class SubPixelTrainer(NormalTrainer):
    def __init__(self, config, training_loader, testing_loader):
        super(SubPixelTrainer, self).__init__(config, training_loader, testing_loader)

        self.model = Net(in_channel=2, upscale_factor=self.upscale_factor).to(self.device)
