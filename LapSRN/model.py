...
class FeatureExtraction(nn.Module):
    def __init__(self, level, in_channel=2):
        super(FeatureExtraction, self).__init__()
        if level == 1:
            self.conv0 = nn.Conv2d(in_channel, 64, (3, 3), (1, 1), (1, 1))
        else:
...
class ImageReconstruction(nn.Module):
    def __init__(self, in_channel=1):
        ...
        self.conv_R = nn.Conv2d(64, in_channel, (3, 3), (1, 1), (1, 1))
        self.convt_I = nn.ConvTranspose2d(in_channel, in_channel, (4, 4), (2, 2), (1, 1))
...
class LasSRN(nn.Module):
    def __init__(self, in_channel=1, upscale_factor=2):
        super(LasSRN, self).__init__()
        self.upscale_factor = upscale_factor
        self.FeatureExtraction1 = FeatureExtraction(level=1, in_channel=in_channel)
        self.FeatureExtraction2 = FeatureExtraction(level=2)
        self.FeatureExtraction3 = FeatureExtraction(level=3)
        self.ImageReconstruction1 = ImageReconstruction(in_channel=in_channel)
        self.ImageReconstruction2 = ImageReconstruction(in_channel=in_channel)
        self.ImageReconstruction3 = ImageReconstruction(in_channel=in_channel)
