...
from CSNLA.model import CSNLN
from Lib.MetaLib import NormalTrainer


class CSNLATrainer(NormalTrainer):
    def __init__(self, config, training_loader, testing_loader):
        super(CSNLATrainer, self).__init__(config, training_loader, testing_loader)

        self.model = CSNLN(num_channels=2, upscale_factor=self.upscale_factor).to(self.device)
