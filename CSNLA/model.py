...
class MultisourceProjection(nn.Module):
    def __init__(self, in_channel, kernel_size=3, scale=2, conv=default_conv):
        ...
        self.up_attention = CrossScaleAttention(scale=up_factor, channel=in_channel)
...
class CSNLN(nn.Module):
    def __init__(self, num_channels=2, upscale_factor=2, conv=default_conv):
        ...
        # define head module
        m_head = [
            BasicBlock(conv, num_channels, n_feats, kernel_size, stride=1, bias=True, bn=False, act=nn.PReLU()),
...
if __name__ == '__main__':
    ...
    model = CSNLN(num_channels=2, upscale_factor=2).float().to(device)
