...
class SRGANTrainer(object):
    ...
    def build_model(self):
        self.netG = Generator(n_residual_blocks=self.num_residuals, upsample_factor=self.upscale_factor, base_filter=64, num_channel=2).to(self.device)
    ...
    def save(self):
        g_model_out_path = "%s_Generator_model_path.pth"%self.type
        d_model_out_path = "%s_Discriminator_model_path.pth"%self.type
    ...
    def pretrain(self):
        ...
        for batch_num, (data, _, target) in enumerate(self.training_loader):
            ...
            total_loss += loss
            progress_bar(batch_num, len(self.training_loader), 'Loss: %.4f' % (total_loss / (batch_num + 1)))
