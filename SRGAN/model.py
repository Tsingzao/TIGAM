...
class UpsampleBlock(nn.Module):
    ...
    def __init__(self, in_channels, scale=2):
        ...
        self.conv = nn.Conv2d(in_channels, in_channels * (scale**2), kernel_size=3, stride=1, padding=1)
...
class Generator(nn.Module):
    def __init__(self, n_residual_blocks, upsample_factor, num_channel=1, base_filter=64):
        ...
        if self.upsample_factor == 2 or self.upsample_factor == 3 or self.upsample_factor == 5:
            self.add_module('upsample' + str(self.upsample_factor), UpsampleBlock(base_filter, self.upsample_factor))
        if self.upsample_factor == 4:
            self.add_module('upsample2', UpsampleBlock(base_filter, 2))
            self.add_module('upsample4', UpsampleBlock(base_filter, 2))
    ...
    def forward(self, x):
        ...
        if self.upsample_factor == 2 or self.upsample_factor == 3 or self.upsample_factor == 5:
            x = self.__getattr__('upsample' + str(self.upsample_factor))(x)
        if self.upsample_factor == 4:
            x = self.__getattr__('upsample2')(x)
            x = self.__getattr__('upsample4')(x)
